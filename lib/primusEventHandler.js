...
module.exports = function (conf, models, primus) {
	...
	primus.on('connection', function (spark) {
		...
		spark.on('changetopic', function(roomHash, fn){
			var r = roomHash || {};
			var room = r.room;
			chatService.updateRooms(spark.customerId, [room], [{id: room, topic: r.topic}], fn);
		});

		spark.on('listrooms', function(queryHash, fn){
			var queryHashIsFunction = _.isFunction(queryHash);
			var cb = queryHashIsFunction ? queryHash : fn;
			
			chatService.withCustomerRooms(spark.customerId, function(rooms){
				cb && cb(rooms);
			});
		});
		...
		spark.on('typing', function(hash, fn){
			var senderId = spark.userId;
			chatService.withUserRooms(spark.customerId, senderId, function(rooms, err){
				if(!!hash.room && _.contains(rooms, hash.room)){
					chatService.broadcastIsTyping(spark.customerId, hash.room, senderId, hash.is_typing || false);
					fn && fn(true);
				else {
					logger.info("Spark claims to be typing in to room " + hash.room + " which he hasn't joined!");
					fn && fn(false);
				}
			});
		});
	...
	function msgToRoom(spark, msg, fn){
		...
		chatService.withUserRooms(spark.customerId, senderId, function(rooms, err){
			if(!!msg.room && _.contains(rooms, msg.room)){
				// publish to redis
				chatService.broadcastChatMessage(spark.customerId,
					_.extend(
						msg, 
						{ 
