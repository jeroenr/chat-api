...
module.exports = function (conf, models, primus) {
	  ...
	  if(!!token){
		  try {
		    ...
		    if (decoded.exp <= Date.now()) {
		      return done({statusCode: 400, message: 'Access token has expired'});
			}

			customerService.get(decoded.iss, function(body, success){
				if(success && decoded.iss === id) {
					// you are clear
					return done();
				} else {
					throw new Error('Unknown customer ' + decoded.iss); 
				}
			});
		  } catch (err) {
		  	logger.error('Authentication error ' + util.inspect(err));
		    return done({statusCode: 401, message: 'Authentication error'});
		  }
	...
	primus.on('connection', function (spark) {
		...
		spark.on('msgtousers', function(msg, fn){
			...
			if(_.isArray(msg.ids)){
				...
				chatService.joinAllRooms(spark.userId, [room], function(){
					...
					addPeople(spark.userId, room, msg.ids, function(added){
		...
		spark.on('addpeople', function(hash, fn) {
			var h = hash || {};
			addPeople(spark.userId, h.room, h.ids, function(success){
				fn && fn(success);
			});
		});
