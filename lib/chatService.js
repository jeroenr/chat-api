...
module.exports = function (conf, models, primus) {
	...
	var redisIsTypingChannel = models.redis.isTypingChannel;
	...
	function deliverIsTyping(customerId, isTypingMsg) {
		logger.info("Delivering is typing message " + util.inspect(isTypingMsg));
		withRooms(ROOMS.format(customerId, isTypingMsg.room), function(resp, err){
			var uuidsInRoom = resp || [];
			//intersect with connected clients to ensure we only send to clients this server is responsible for
			//broadcast
			var recipients = _.uniq(uuidsInRoom);
			logger.info("Typing recipients: " + util.inspect(recipients));
			primus.forEach(function (spark, id, connections) {
				if (_.contains(recipients, spark.userId)){
				  spark.send('typing', isTypingMsg);
				}
			});
		});
	}

	function broadcastChatMessage(customerId, msg, except) {
	...
	function broadcastIsTyping(customerId, room, userId, isTyping) {
		redisClient.publish('istyping',
			JSON.stringify(
				{ 
					customer_id: customerId, 
					user_id: userId,
			...
			logger.info("Got redis response " + util.inspect(replies) + " error: " + util.inspect(err));
     		...
     		if(!silent){
	     		_.each(rooms, function(r){ 
	     			broadcastChatMessage(customerId,
	         			{
	         				room: r.id, 
	...
	function updateRooms(customerId, roomIds, roomHashes, fn) {
		var roomKeys = _.map(roomIds, function(roomId){
			return ROOM_PROPS.format(customerId, roomId);
		});
		var roomsAsJson = _.map(roomHashes, function(roomHash){
			return JSON.stringify(roomHash);
		});
		redisClient.mset(_.flatten(_.zip(roomKeys, roomsAsJson)), function(err, replies){
			fn(!err);
		});
	}
	...
	redisChatMessageChannel.on('message', function(channel, message){
		logger.info("Server received broadcasted chat message: " + util.inspect(message));
		var msg = JSON.parse(message);
		deliverChatMessage(msg.customer_id, msg, msg.excluded);
	});

	redisIsTypingChannel.on('message', function(channel, message){
		logger.info("Server received broadcasted is typing message: " + util.inspect(message));
		var msg = JSON.parse(message);
		deliverIsTyping(msg.customer_id, msg);
	});
	...
	return {
		...
		deliverIsTyping: deliverIsTyping,
